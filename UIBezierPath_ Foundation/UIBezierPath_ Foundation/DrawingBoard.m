...
#define radianToDegress(radian) (radian * 180.0f)/(M_PI)
...
- (void)drawRect:(CGRect)rect {
    ...
    //以某个中心点画弧线
    UIBezierPath *arcPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(206, 15) radius:20 startAngle:0 endAngle:degressToRadian(200) clockwise:YES];
    [brushColor set];
    [arcPath stroke];
    
    //添加一个弧线
    UIBezierPath *arcPath2 = [UIBezierPath bezierPath];
    [arcPath2 moveToPoint:CGPointMake(240, 30)];
    [arcPath2 addArcWithCenter:CGPointMake(265, 30) radius:25 startAngle:degressToRadian(180) endAngle:degressToRadian(360) clockwise:YES];
    //添加一个UIBezierPath
    ...
    //跟胡CGPath创建并返回一个新的UIBezierPath对象
...
- (UIBezierPath *)bezierPathWithCGPath {
    ...
    CGRect recttangle = CGRectInset(CGRectMake(0, 0, CGRectGetWidth(self.bounds), CGRectGetWidth(self.bounds)), 3, 3);
    ...
    CGPathAddRoundedRect(path, NULL, recttangle, 5, 5);
